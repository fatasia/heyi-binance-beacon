...
A bilingual, chain-only visualizer for the HE YI TapeOut circuit. Every displayed frame comes from a read-only BNB Chain call; there is no local demo fallback.
...
The page validates the two bytes returned by the processor and decodes both the ASCII letter and Binance direction signature.
...
Each call is pinned to the block height shown in the on-page call receipt. The receipt also exposes the processor address, circuit ID, method, requested state, returned state, raw output and RPC source. If every configured RPC is unavailable, the page freezes the last verified frame and does not synthesize another output locally.
